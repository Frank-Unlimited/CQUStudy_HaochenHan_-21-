...
public class event {
    ...
    public boolean login(int id, String password){
        try {
            ...
            //等待服务端处理
            long start = System.currentTimeMillis();
            //持续监测id为3的消息队列，处理登录事件的队列  检测消息队列中是否有服务器处理登录事件的消息。如果队列为空，表示服务器还未处理完登录事件。
    ...
    public void transfor_broad(int id,int lenToID,int [] toID, String dataType,byte[] data){
        /*
         * 发送事件封装成协议，字段说明如下
         * @ 10: 发送事件标识字段
         * @ id: 客户端id
         ...
         * @ data: 具体封装的数据
         */
        clientProtocol message = new clientProtocol(88, id, lenToID, toID, dataType, data);
        clientSocket.queue.write(0, message);
    }

    /**********************************申请加群*************************************/
    public boolean joinGroup(int id,int gid,byte[] data) {
        /*
         * 发送事件封装成协议，字段说明如下
         * @ 10: 发送事件标识字段
         * @ id: 客户端id
         ...
         */
        try {
            int[] toID = new int[]{gid};
            clientProtocol message = new clientProtocol(33, id, 1, toID, 0, data);
            clientSocket.queue.write(0, message);
            //等待服务端处理
            long start = System.currentTimeMillis();
            //持续监测id为3的消息队列，处理登录事件的队列  检测消息队列中是否有服务器处理登录事件的消息。如果队列为空，表示服务器还未处理完登录事件。
            while(clientSocket.queue.isEmtry(44)){
                /*
                 * 同上
                 */
                if(System.currentTimeMillis()-start > time){ break; }
            ...
            if(clientSocket.queue.isEmtry(44)){
                return false;
            } else {
                // 服务器端会返回登录是否成功
                clientProtocol message1 = clientSocket.queue.read(44);
                // 清空
                while(!clientSocket.queue.isEmtry(44)){ clientSocket.queue.read(44); }
                if(new String(message1.getData(),"utf-8").equals("success")){ return true; }
                return false;
            }
    ...
    /**********************************拉人加群*************************************/
    public void inviteGroup(int id,int lenToID, int [] toID, byte[] data) {
        clientProtocol message = new clientProtocol(71, id, lenToID, toID, 0, data);
        clientSocket.queue.write(0, message);
    }

    /**********************************申请退群*************************************/
    public boolean quitGroup(int id,int gid,byte[] data) {
        /*
         * 发送事件封装成协议，字段说明如下
         * @ 10: 发送事件标识字段
         * @ id: 客户端id
        ...
        try {
            int[] toID = new int[]{gid};
            clientProtocol message = new clientProtocol(55, id, 1, toID, 0, data);
            clientSocket.queue.write(0, message);
            //等待服务端处理
            long start = System.currentTimeMillis();
            //持续监测id为3的消息队列，处理登录事件的队列  检测消息队列中是否有服务器处理登录事件的消息。如果队列为空，表示服务器还未处理完登录事件。
            while(clientSocket.queue.isEmtry(77)){
                /*
                 * 同上
                 */
                if(System.currentTimeMillis()-start > time){ break; }
            ...
            if(clientSocket.queue.isEmtry(77)){
                return false;
            } else {
                // 服务器端会返回登录是否成功
                clientProtocol message1 = clientSocket.queue.read(77);
                // 清空
                while(!clientSocket.queue.isEmtry(77)){ clientSocket.queue.read(77); }
                if(new String(message1.getData(),"utf-8").equals("success")){ return true; }
                return false;
            }
        } catch (UnsupportedEncodingException e) {
    ...
    /**********************************发送群聊消息*************************************/
    public void transfor_group(int id,int gid, String dataType,byte[] data){
        int [] toID = new int[]{gid};
        clientProtocol message = new clientProtocol(66, id, 1, toID, dataType, data);
        clientSocket.queue.write(0, message);
    }
