            ...
            while (!isClose) {
                ...
                for (clientProtocol message : relist) {
                    ...
                    for (int i = 0; i < message.getLenToID(); i++) {
                    	/*
                    	 * 注意：由服务器转发的协议类型主要是事件回复类型和转发消息类型，其ID为3 4 5（4为注销回复，没必要回复）
                        ...
                        if(message.getType() == 72) {
                            System.out.println("拉群成功");
                            //开启对应ID的消息队列
                            serverSocket.Queues.putIfAbsent(message.getToID()[0]);
                            //写线程启动
                            if(!isOpenWrite) {
                        ...
                        if(message.getType()==99) {
                            System.out.println("公屏广播请求, 数据类型为：");
                            online onlineManager = new online();
                        ...
                        /*****************************群聊回复消息*****************************/
                        if(message.getType()==166) {
                            System.out.println("群聊广播请求, 数据类型为：");
                            System.out.println(message.getDatatype()+message.getData());
                            if (serverSocket.Queues.isExist(message.getToID()[i])) {
                                System.out.println("转发请求");
                                //将该消息写入对应ID队列中
                                serverSocket.Queues.write(message.getToID()[i], message);
                        ...
                        /*****************************转发消息*****************************/
                        if(message.getType()!=99 && message.getType()!=166) {
                            if (serverSocket.Queues.isExist(message.getToID()[i])) {
                                System.out.println("转发请求");
                                //将该消息写入对应ID队列中
